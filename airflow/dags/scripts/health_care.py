...
from scripts.datacube import BASE_URI, RVO, create_health_care_QB, add_region_instance, add_county_instance, add_field_of_care_instance, write
...
def create_health_care_datacube(csv_data_path, output_path = None):
...
def main():
    ...
    if len(sys.argv == 2):
        create_health_care_datacube(csv_data_path = sys.argv[1])

    else:
        create_health_care_datacube(casv_data_path = sys.argv[1], output_path = sys.argv[2])
